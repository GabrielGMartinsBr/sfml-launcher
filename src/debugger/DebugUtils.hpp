...
struct DebugUtils {
  static const char* className(VALUE classObj)
  {
  ...
  static const char* classPath(VALUE classObj)
  {
    return Convert::toCStr(
  ...
  static const char* methodName(ID mid)
  {
  ...
  static String getDebugVars(VALUE self, ID mid, VALUE classObj)
  {
    ...
    strStream << "globalVars:" << globalVars.size() << '|' << globalVars;

    String localVars = getLocalVariablesDebug();
    strStream << "localVars:" << localVars.size() << '|' << localVars;

    if (TYPE(classObj) == T_FALSE) {
      return strStream.str();
    }

    String className = DebugUtils::className(classObj);
    className += '|';
    String classPath = DebugUtils::classPath(classObj);
    classPath += '|';
    strStream << "className:" << className.size() << '|' << className;
    strStream << "classPath:" << classPath.size() << '|' << classPath;

    String classVars = getClassVariablesDebug(classObj);
    strStream << "classVars:" << classVars.size() << '|' << classVars;
    ...
    strStream << "instanceVars:" << instVars.size() << '|' << instVars;
